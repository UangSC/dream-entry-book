...
5. 登录后开始阅读并保存进度，再点击退出，预期回到首页并恢复“知乎登录”。未登录点击开始、继续、书架、导入或设置应提示授权；重新登录后可继续原进度。取消知乎授权应展示可返回作品的提示，无无限重试。应用会话最长一小时，过期后重新登录。
6. 阅读期间断网，切回标签页触发会话检查，或等待可见页面的下一次定期检查（每 60 秒）；检查失败后应回首页并保留本地存档。恢复网络、确认授权后应能继续阅读。

通过标准：健康接口 200、新前端正常加载、一次真实授权返回正确账号、刷新保留会话、退出与会话检查失败后关闭受限入口、重新授权后可继续原进度。本地书签不是授权凭据，素材缓存不保证断网时仍可阅读。
