...
describe('章节标题和品牌共用的演出时钟', () => {
  it('标题在 3–8 秒消散，品牌在 5.5–10.5 秒随后退场', () => {
    vi.useFakeTimers();
    ...
    vi.advanceTimersByTime(2499); expect(onBrand).not.toHaveBeenCalled();
    vi.advanceTimersByTime(1); expect(onBrand.mock.calls).toEqual([['ash']]);
    vi.advanceTimersByTime(2499); expect(onPhase).toHaveBeenCalledTimes(1);
    vi.advanceTimersByTime(1); expect(onPhase.mock.calls).toEqual([['ash'], ['hidden']]);
    expect(onBrand).toHaveBeenCalledTimes(1);
    vi.advanceTimersByTime(2499); expect(onBrand).toHaveBeenCalledTimes(1);
  ...
  it('换章取消旧时钟，不让前章提前隐藏新章标题', () => {
    ...
    vi.advanceTimersByTime(7500); expect(oldChapter).not.toHaveBeenCalled(); expect(oldBrand).not.toHaveBeenCalled(); expect(nextChapter).toHaveBeenLastCalledWith('hidden');
