...
import { ZhihuSources } from './ZhihuSources';
...
export function LibraryHost({ children }: { children: (data: GameData, actions: LibraryActions) => ReactNode }) {
  ...
  const [books, setBooks] = useState<BookRecord[]>([]), [panel, updatePanel] = useState<'shelf' | 'add' | 'generate' | null>(null);
  const setPanel = (next: typeof panel) => { if (next === null || requireLogin()) updatePanel(next); };
  useEffect(() => { if (!user) updatePanel(null); }, [user]);
  ...
  const run = async (label: string, task: () => Promise<void>) => {
    ...
    try { await new Promise<void>(resolve => requestAnimationFrame(() => resolve())); if (owner && owner === accountId.current && requireLogin()) await task(); }
  ...
  const choose = (key: string) => void run('正在翻开入梦书', async () => {
    ...
    if (!owner || owner !== accountId.current || !requireLogin()) return;
