...
export function StoryText({ beat, motion, instant, onComplete, speed, mode = 'authored' }: { beat: Beat; motion: boolean; instant: boolean; onComplete: () => void; speed: number; mode?: TextAnimation }) {
  ...
  const duration = effect === 'typewriter' ? Math.max(320, ([...beat.text].length - 1) * speed + 320) : beat.effect?.durationMs ?? (beat.effect ? 850 : 450);
  useEffect(() => {
  ...
  return <p className={`story-text effect-${effect} ${beat.kind === 'thought' ? 'thought-text' : ''}`} style={{ '--effect-duration': `${duration}ms` } as CSSProperties}>
    {split ? <><span className="sr-only">{beat.text}</span><span aria-hidden="true">{[...beat.text].map((char, index) => <span className="letter" key={index} style={{ '--letter-index': index, '--letter-delay': `${index * (effect === 'typewriter' ? speed : 8)}ms`, '--scatter-x': `${((index * 7) % 13 - 6) * 8}px`, '--scatter-y': `${((index * 11) % 9 - 4) * 9}px` } as CSSProperties}>{char}</span>)}</span></> : beat.text}
  </p>;
}
