...
interface Preferences { motion: boolean; particles: boolean; audioEnabled: boolean; textAnimation: 'authored' | 'typewriter' | 'instant'; pulse: 'subtle' | 'normal' | 'strong'; fontSize: number; textSpeed: number; master: number; bgm: number; sfx: number; ambience: number }
const DEFAULTS: Preferences = { motion: true, particles: true, audioEnabled: true, textAnimation: 'authored', pulse: 'normal', fontSize: 22, textSpeed: 24, master: .8, bgm: .75, sfx: .45, ambience: .18 };
...
function Game({ data, library }: { data: GameData; library: LibraryActions }) {
  ...
  const beat = state && node ? node.beats[state.beatIndex] : null;
  ...
  const activeTrack = screen === 'reading' ? (view?.silenced ? null : view?.track === 'BGM_DREAM' ? presentation.dreamMusic ?? null : view?.track ?? presentation.dreamMusic ?? null) : screen === 'ending' ? presentation.endingMusic ?? null : 'BGM_GATE';
  const sceneProfile = presentation.scenes[activeScene];
  const ambienceTrack = screen === 'reading' ? sceneProfile?.ambience : undefined;
  const endings = pkg.nodes.filter(n => n.kind === 'ending');
  ...
  useEffect(() => {
    ...
    const visibility = () => { if (document.hidden) { void audio.pause(); void ambienceAudio.pause(); } else { void audio.resume(); void ambienceAudio.resume(); } };
  ...
  const choices = node?.kind === 'scene' && state?.phase === 'choosing' ? visibleChoices(node, state) : [];
  ...
  return <div className={`game screen-${screen} ${motion ? 'motion-on' : 'motion-off'} ${portal.busy ? 'portal-busy' : ''}`} style={{ '--reading-size': `${settings.fontSize}px` } as CSSProperties}>
    ...
    {screen === 'reading' && state && node && beat && <main ref={readerRef} className="reading-layout" tabIndex={-1} aria-label="故事阅读" data-node={node.id} data-beat={beat.id} data-phase={state.phase}>
      <div className="chapter-heading"><span className="eyebrow">{presentation.tags[1] ?? '梦中来信'} · {pkg.title}</span><h1>{chapterNames[node.id] ?? '梦中一页'}</h1><span className="chapter-decoration" aria-hidden="true">✧</span><div>{sourceCredit}</div></div>
      <section className="reading-dock"><div className="reading-status"><span className="scene-location"><i />{sceneProfile?.label ?? '梦中一隅'}</span><span>{savedOk ? '书签已收好' : '进度尚未保存'}</span></div>
        <div className="dialogue-card" onClick={event => { if (!(event.target as HTMLElement).closest('button, a')) advanceStory(); }}>
          <div className="speaker-line"><span className={`speaker-avatar speaker-${beat.speaker}`}>{beat.speaker === 'narrator' ? <Icon name="book" size={18} /> : pkg.characters.find(c => c.id === beat.speaker)?.name.slice(-1)}</span><span>{beat.speaker === 'narrator' ? '梦中一页' : pkg.characters.find(c => c.id === beat.speaker)?.name}<small>{beat.kind === 'thought' ? '你的心声' : beat.speaker === pkg.characters[0]?.id ? '此刻，故事因你继续' : beat.kind === 'narration' ? '故事正在发生' : ''}</small></span><span className="page-number">{String(view?.lines.length ?? 1).padStart(2, '0')}</span></div>
          <div className="text-area" role="log" aria-live="polite" aria-atomic="true"><StoryText key={beat.id} beat={beat} motion={motion} instant={instant} speed={settings.textSpeed} mode={settings.textAnimation} onComplete={completeText} /></div>
          {choices.length && ready ? <div className="choices" aria-label="此刻的选择">{choices.map((option, index) => <button key={option.id} className="choice-button" onClick={() => choose(option.id, state.revision)}><span className="choice-number">0{index + 1}</span><span>{option.text}</span><Icon name="arrow" size={18} /></button>)}</div> : <div className="continue-row"><span>{ready ? state.phase === 'finished' ? '这场梦，停在这一页' : '点击文字或按空格，继续阅读' : '点击即可显示完整文字'}</span><button className="continue-button" onClick={advanceStory}>{!ready ? '显示全文' : state.phase === 'finished' ? '收下这场梦' : '继续'}<Icon name="arrow" size={18} /></button></div>}
        </div><div className="reader-toolbar"><div className="progress-track" aria-label="故事阅读进度"><i style={{ width: `${progress}%` }} /></div><button onClick={() => setPanel('history')}><Icon name="history" size={16} />前情回顾</button><button onClick={() => setPanel('chapters')}><Icon name="bookmark" size={16} />分歧回望</button><button onClick={leaveDream}><Icon name="home" size={16} />保存离梦</button></div>
      </section>
    </main>}
    ...
    <footer className="footer"><span><i className={offline === '离线就绪' ? 'status-dot ready' : 'status-dot'} />{offline}</span>{availableUpdate && screen === 'home' ? <button onClick={() => { if (stateRef.current) save(stateRef.current); availableUpdate(); }}>新版本已备好，更新书页</button> : <span>一书一梦 · 不必急着醒来</span>}<a className="zhihu-discover" href="https://www.zhihu.com/" target="_blank" rel="noopener noreferrer">去知乎，发现更多故事 ↗</a><button onClick={() => setPanel('source')}>关于这场梦</button></footer>
