...
function play(choices: string[]) {
  ...
  for (let step = 0; step < 150; step++) {
    expect(validateSave(pkg, state), `节点 ${state.nodeId} 拍 ${state.beatIndex}`).toBeNull();
    expect(replayView(pkg, state).ok).toBe(true);
...
describe('首篇完整路线', () => {
  for (const first of ['feed-fire', 'name-price']) for (const second of ['tell-limit', 'keep-front']) for (const third of ['stay-tonight', 'return-tomorrow']) {
    it(`${first} → ${second} → ${third} 可通关、每拍都可恢复`, () => {
      const state = play([first, second, third]);
      expect(state.choiceHistory).toHaveLength(3);
      expect(state.nodeId).toBe(third === 'stay-tonight' ? 'ending-lantern' : 'ending-path');
...
describe('回滚与存档保护', () => {
  it('回滚恢复此前事实，梦册保留，旧选择令牌不可重用', () => {
    const state = play(['feed-fire', 'tell-limit', 'stay-tonight']);
    ...
    expect(restored.flags.warmed).toBe(false); expect(restored.flags.honest).toBe(false);
    ...
    const token = { nodeId: restored.nodeId, choiceId: 'name-price', nodeVisit: 1, revision: restored.revision };
  ...
  it('篡改事实和检查点都不能伪装成可续读存档', () => {
    const good = play(['feed-fire', 'tell-limit', 'return-tomorrow']);
...
describe('制作期拦截无法通关的故事', () => {
  it('拒绝普通 next 形成的循环', () => {
    const broken = structuredClone(pkg); const node = broken.nodes.find(n => n.id === 'small-miracle')!;
  ...
  it('拒绝有连线但永远无法执行到的终幕', () => {
    const broken = structuredClone(pkg); const node = broken.nodes.find(n => n.id === 'last-light')!;
    if (node.kind === 'scene') node.choices![0]!.when = { all: [{ kind: 'flag', id: 'stayed', equals: true }] };
